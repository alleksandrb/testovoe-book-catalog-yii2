...
<div class="book-update">
    ...
    <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data']]); ?>
        ...
        <?php
        $selectedAuthors = \yii\helpers\ArrayHelper::getColumn($book->authors, 'id');
        ?>
        <div class="form-group">
            <label>Authors</label>
            <?php foreach (Author::find()->all() as $author): ?>
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="authorIds[]" value="<?= $author->id ?>" id="author_<?= $author->id ?>" <?= in_array($author->id, $selectedAuthors) ? 'checked' : '' ?>>
